...
| View franchise<br/>(as diner)                        | `franchiseDashboard.tsx`  | `GET /api/franchise/:userId`                       | `SELECT objectId FROM userRole WHERE role='franchisee' AND userId=?`<br/>(empty result → marketing page) |
...
| View Admin page                                      | `adminDashboard.tsx`      | `GET /api/franchise?page=&limit=&name=`            | `SELECT id, name FROM franchise WHERE name LIKE ? LIMIT ? OFFSET ?` |
